...
| `not_available` | Live bank details or onramp not ready |
...
- `400` `transfer_failed` with insufficient funds — [Receive](/receive) onto the customer account, then retry with a **new** idempotency key only if you intend a new send.
