...
Create the transfer with your secret key. The customer authorizes it — in your app with the `client_secret`, or on the hosted review URL. If they do not, the funds stay in the vault. A merchant key cannot send on their behalf.
...
<Steps>
  <Step title="Fund the vault">
    `available` must cover the send. Fund with [Receive](/receive). In test: Console **Simulate test deposit**, or complete a test onramp. See [Test and live](/test-and-live).
  </Step>
  ...
  <Step title="Transfer">
    `POST /v1/transfers` with `{ "quote": "qt_…" }` and `Idempotency-Key`. Returns `requires_action` and a `client_secret`.
  </Step>
  <Step title="Customer authorizes">
    Show amount and destination. `POST /v1/transfers/:id/confirm` with `{ "client_secret": "easner_ta_…" }` — not your secret key. Or open `next_action.url`.
  </Step>
  <Step title="Fulfil from webhooks">
    `transfer.completed` and `transaction.created`. Do not treat HTTP `201` as “the recipient has the money”.
  </Step>
</Steps>
...
| Mode | What happens |
|---|---|
| Test | Create returns `requires_action`. Confirm debits the **test** vault. No live rails move |
| Live | Create returns `requires_action` and locks `available` into `pending`. Confirm sends from the customer vault. A merchant key on confirm is `403` `customer_action_required` |

Pick a corridor from [Payment methods](/payment-methods) (`GET /v1/payment_methods`), then create a destination with those details, quote, transfer.

Checkout is not a send funding method and not a destination. A website payment does not add to this `acct_`.
...
<Card title="Payment methods" icon="list" href="/payment-methods">
  Rails, corridors, and how they map to destination details.
</Card>
