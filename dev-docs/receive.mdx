...
Live virtual accounts, live wallet addresses, and onramp require hosted [KYC](/customers#start-kyc) on that `cus_`. Your own KYB in Banking Settings does not approve the customer. A `cus_` with only email and name cannot hold a **named** virtual account. Test skips KYC.
...
<Steps>
  ...
  <Step title="Start KYC">
    `POST /v1/customers/:id/verification` with `{ "return_url": "https://…" }`. Test returns `{ "status": "approved", "url": null }`. Live returns a hosted KYC `url`. See [Customers](/customers).
  </Step>
