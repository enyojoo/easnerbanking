...
It is not Checkout. Checkout collects onto **your** business balance.
...
Live virtual accounts, live wallet addresses, and onramp require hosted [verification](/customers#start-verification). A `cus_` with only email and name cannot hold a **named** virtual account. Test skips verification.
...
| Mode | Result |
|---|---|
| Test | Sandbox numbers. No KYC. Nothing moves on a real bank |
...
| Product | Lands on |
|---|---|
| Receive (`deposit` / `chain` / `onramp`) | Customer vault (`acct_` with `customer`) |
| Checkout | **Your** business balance |
