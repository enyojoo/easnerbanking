...
**`customer.created`** / **`customer.updated`** — `data` is the [Customer](/customers). `customer.updated` fires when `verification_status` changes.

**`transfer.created`** / **`transfer.completed`** / **`transfer.failed`** — `data` is the [Transfer](/transfers). Failed includes `error`.

**`transaction.created`** — `data` is the [Transaction](/transactions). Receive types are `deposit`, `onramp`, and `chain`.
