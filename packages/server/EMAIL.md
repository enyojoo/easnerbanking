...
| `POST /api/settings/push-token` | **business** | Body `{ expoPushToken: string \| null }` — register or clear the Expo push token on `users.expo_push_token` (mobile). Authenticated user only; never log the token. |
