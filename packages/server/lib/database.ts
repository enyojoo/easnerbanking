...
export const userService = {
  ...
  async updateProfile(
    ...
    if (user?.noah_kyc_status === "approved") {
    ...
    console.log(`[USER-SERVICE] updateProfile called for user ${userId}:`, {
      updates,
      kycStatus: user?.noah_kyc_status,
    ...
    if (updates.firstName !== undefined && user?.noah_kyc_status !== "approved") {
      updateData.first_name = updates.firstName
      console.log(`[USER-SERVICE] Adding first_name to update:`, updates.firstName)
    }
    if (updates.middleName !== undefined && user?.noah_kyc_status !== "approved") {
      ...
      console.log(`[USER-SERVICE] NOT adding middle_name:`, {
        middleNameUndefined: updates.middleName === undefined,
        kycApproved: user?.noah_kyc_status === "approved",
        middleNameValue: updates.middleName,
      })
    }
    if (updates.lastName !== undefined && user?.noah_kyc_status !== "approved") {
  ...
  async getStats() {
    ...
    // Count users with approved Noah KYC status
...
export const adminService = {
  ...
  async getAllUsers(
    ...
    if (filters.verification && filters.verification !== "all") {
      ...
      const noahKycStatus = statusMap[filters.verification] || filters.verification
      query = query.eq("noah_kyc_status", noahKycStatus)
    }
...
export const cryptoReceiveTransactionService = {
  async create(
    ...
    providerFields?: {
      ...
      /** Noah API: destination can include card settlement / funding rails */
    ...
    if (providerFields) {
      Object.assign(insertData, providerFields)
    }
