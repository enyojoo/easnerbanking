...
| `TURNKEY_API_PUBLIC_KEY` | API key pair (server) — same public key is registered on each **new** sub-org as root user “Easner provisioner” so `createWallet` stamps match the sub-org (fixes `ORGANIZATION_MISMATCH`) |
...
3. **Link to Easner** — **`POST /api/auth/bootstrap`** calls Turnkey when parent org API keys are set; clients also call **`POST /api/wallets/ensure-sub-org`** right after a successful bootstrap (idempotent) so legacy accounts and earlier transient failures still get a sub-org linked. Business web must send **`X-Easner-Noah-Scope: business`**. Manual / embedded flows can still call **`POST /api/wallets/provision`** with `{ "turnkeySubOrganizationId": "<id>" }`. Disable auto-create with **`TURNKEY_SERVER_SUB_ORG_CREATION_ENABLED=false`**. Each **new** sub-org includes root user **Easner provisioner** carrying your server API public key so vault jobs can `createWallet` without **`ORGANIZATION_MISMATCH`**. Sub-orgs created **before** that change need the same API user added in the Turnkey UI (matching public key + curve) or a new sub-org via ensure/bootstrap.
