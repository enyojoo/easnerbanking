...
export function resolveBankDepositPayInDetail(
  ...
  if (!isNoahBankOnrampFiatPayIn(payload)) return null

  const enrichment = extractNoahBankPayInEnrichment(payload)
  if (!enrichment) return null

  const fiatDepositId = pickFiatDepositId(payload, meta, enrichment)
  ...
  const depositAmount = roundFiat(
    typeof meta.fiat_deposit_amount === "number"
      ...
      : enrichment.fiatAmount,
  ) ?? enrichment.fiatAmount

  const feeAmount = roundFiat(
    typeof meta.fee_amount === "number" ? meta.fee_amount : enrichment.feeAmount,
  )
  ...
  const postedCurrency =
    meta.settled_currency != null
      ...
      : enrichment.walletLedgerCurrency ?? enrichment.fiatCurrency
  ...
  const payInFields: Record<string, unknown> = {
    ...
    noah_rule_execution_id: enrichment.ruleExecutionId ?? fiatDepositId,
