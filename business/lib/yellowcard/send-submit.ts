...
export function buildYcSendSubmitBody(input: YcSendSubmitInput): Record<string, unknown> {
  ...
  if (directSettlement && input.settlementCryptoAmount != null) {
    // Preserve USDC precision in the request. The lock treats YC's returned cryptoAmount as
    // authoritative and detects whether YC actually preserves or quantizes this value.
