...
export async function recordBridgeVerificationOutcome(
  ...
  await persistVerificationStatus(admin, {
    ...
    rejectionReasons: businessId ? undefined : stored,
  ...
  const previousEmail =
    nextEmail === "rejected" && bridgeStatusForEmail(previousStatus) === "rejected" && !hadReasons
      ? "not_started"
      : bridgeStatusForEmail(previousStatus)
