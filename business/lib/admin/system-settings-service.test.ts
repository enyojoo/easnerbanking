...
describe("system-settings-service allowlist", () => {
  it("allows platform access and USD/EUR active keys", () => {
    ...
    expect(isAllowedSystemSettingKey("min_native_version_personal")).toBe(true)
  ...
  it("normalizes booleans and email provider", () => {
    ...
    expect(normalizeSystemSettingValue("min_native_version_personal", "1.10.2").value).toBe("1.10.2")
    expect(normalizeSystemSettingValue("min_native_version_personal", "").value).toBe("")
    expect(() => normalizeSystemSettingValue("min_native_version_personal", "latest")).toThrow(/1\.10\.2/)
