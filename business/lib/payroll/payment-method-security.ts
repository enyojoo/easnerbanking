...
export function maskPayrollMethodDetails(
  ...
  if (type === "bank") {
    return { bankName: details.bankName || "Bank account", account: tail(details.accountNumber) }
  }
  if (type === "mobile_money") {
    return { provider: details.provider || "Mobile money", phone: tail(details.phoneNumber) }
  }
  return { network: details.network || "Stablecoin", wallet: tail(details.walletAddress) }
}
