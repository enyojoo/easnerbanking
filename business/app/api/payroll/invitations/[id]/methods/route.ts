...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const complete =
    body.type === "bank" ? details.bankName?.trim() && details.accountNumber?.trim() :
    body.type === "mobile_money" ? details.provider?.trim() && details.phoneNumber?.trim() :
  ...
  await admin.from("payroll_payment_methods").update({
    ...
    .eq("owner_type", "employee").neq("type", "easetag").eq("status", "active")
