...
function gridCrossBorderQuoteError(
  code: string,
  error: string,
  status: number,
...
export async function POST(request: Request) {
  ...
  if (!recipientId || !(receiveAmount > 0) || !payInCurrency || !payInCountry) {
    return gridCrossBorderQuoteError(
      "recipient_required",
  ...
  const corridorEnabled = await isGridLocalPayInEnabledForCorridor(admin, {
  ...
  const { data: recipient } = await admin
    ...
    .maybeSingle()
  if (!recipient) {
    return gridCrossBorderQuoteError("recipient_not_found", "Recipient not found", 404)
  }
