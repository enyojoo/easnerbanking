...
**MoMo pattern:** Phone + network collected on setup → **single `/confirm` on setup Continue** (spinner on CTA) → review/confirm screen reads stashed lock with no spinner. Never call `/confirm` while typing a phone number.
