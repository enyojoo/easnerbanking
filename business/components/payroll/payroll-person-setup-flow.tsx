...
export function PayrollPersonSetupFlow() {
  ...
  const steps = useMemo(() => method === "manual"
    ? ["Setup", "Details", "Payment details", "Done"]
    : ["Setup", "Details", "Review", "Done"], [method])
  const activeIndex = step === "method" ? 0 : step === "details" ? 1 : step === "payment" ? 2 : step === "review" ? 2 : 3

  if (!capabilitiesQuery.isPending && !capabilitiesQuery.data?.canPrepare) {
    return <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6"><Card><CardContent className="p-8 text-center"><h1 className="text-lg font-semibold">You can’t add Payroll people</h1><p className="mt-2 text-sm text-muted-foreground">Ask a Payroll preparer or approver to add this person.</p><Button className="mt-5" variant="outline" onClick={() => router.push("/payroll/people")}>Back to People</Button></CardContent></Card></div>
  }
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8 flex items-center gap-2 overflow-x-auto" aria-label="Setup progress">
        {steps.map((label, index) => (
          <div key={label} className="flex shrink-0 items-center gap-2">
            <span className={cn("flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold", index <= activeIndex ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>{index + 1}</span>
            <span className={cn("text-xs", index <= activeIndex ? "text-foreground" : "text-muted-foreground")}>{label}</span>
      ...
      {step === "method" ? (
        <div className="grid gap-4 md:grid-cols-2">
          <MethodCard
            ...
            badge="Recommended"
            onClick={() => { setMethod("easetag"); setStep("details") }}
          />
          <MethodCard
            icon={WalletCards}
            title="Add manually"
      ...
      {step === "details" ? (
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <Card className="shadow-card"><CardContent className="space-y-5 p-6">
            {method === "easetag" ? (
              <div className="space-y-2">
                <Label htmlFor="easetag">EASETAG</Label>
                <div className="flex gap-2">
                  <Input id="easetag" value={lookup} onChange={(e) => { setLookup(e.target.value); setProfile(null); setLookupState("idle") }} placeholder="@easetag" />
                  <Button type="button" variant="outline" onClick={() => void findEasetag()} disabled={lookupState === "loading" || lookup.replace(/^@/, "").length < 4}>
                    <Search className="mr-2 h-4 w-4" />{lookupState === "loading" ? "Checking…" : "Find"}
                  </Button>
                </div>
                {lookupState === "missing" ? <p className="text-sm text-destructive">We couldn’t find that EASETAG.</p> : null}
                {profile ? (
            ...
            {method === "easetag" ? (
              <label className="flex items-start gap-3 rounded-xl border p-3 text-sm">
                <input type="checkbox" checked={sendInvitation} onChange={(e) => setSendInvitation(e.target.checked)} className="mt-1" />
                <span><span className="font-medium">Send payroll request now</span><span className="block text-xs text-muted-foreground">The request expires after seven days. Saving without sending keeps the person in a needs-attention state.</span></span>
              </label>
            ) : null}
      ...
      {step === "review" && profile ? (
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <Card className="shadow-card"><CardContent className="space-y-5 p-6">
            <div><h2 className="font-semibold">Review payroll request</h2><p className="mt-1 text-sm text-muted-foreground">Nothing is shared until the person approves.</p></div>
      ...
      {step === "success" && created ? (
        <Card className="mx-auto max-w-2xl shadow-card"><CardContent className="p-8 text-center">
          <CheckCircle2 className="mx-auto h-12 w-12 text-emerald-600" />
          <h2 className="mt-4 text-xl font-semibold">{created.fullName} was added</h2>
          <p className="mt-2 text-sm text-muted-foreground">{created.rail === "easetag" && sendInvitation ? "Their payroll request has been sent." : "Their payroll details are ready to review."}</p>
...
function MethodCard({ icon: Icon, title, description, badge, onClick }: { icon: typeof Landmark; title: string; description: string; badge?: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="rounded-2xl border bg-card p-6 text-left shadow-soft transition hover:border-primary/40 hover:shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
    <div className="flex items-start justify-between"><span className="rounded-xl bg-primary/10 p-3 text-primary"><Icon className="h-6 w-6" /></span>{badge ? <span className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">{badge}</span> : null}</div>
    <h2 className="mt-5 font-semibold">{title}</h2><p className="mt-2 text-sm text-muted-foreground">{description}</p>
    <span className="mt-5 inline-flex items-center text-sm font-medium text-primary">Continue <ArrowRight className="ml-1 h-4 w-4" /></span>
  </button>
}
