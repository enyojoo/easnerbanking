...
export function PayrollScheduleForm({ schedule }: { schedule?: PayrollSchedule }) {
  ...
  const [sourceAccountId, setSourceAccountId] = useState(String(template.sourceAccountId || accounts[0]?.id || ""))
  const [sourceCurrency, setSourceCurrency] = useState(String(template.sourceCurrency || accounts[0]?.currency || "USD"))
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
    <Card className="shadow-card"><CardContent className="space-y-7 p-6 sm:p-8">
      <section><h2 className="font-semibold">Schedule details</h2><div className="mt-4 grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Source account"><Select value={sourceAccountId} onValueChange={(id) => { const account = accounts.find((item) => item.id === id); setSourceAccountId(id); setSourceCurrency(account?.currency || sourceCurrency) }}><SelectTrigger><SelectValue placeholder="Choose account" /></SelectTrigger><SelectContent>{accounts.map((account) => <SelectItem key={account.id} value={account.id}>{account.currency} account</SelectItem>)}</SelectContent></Select></Field>
      </div></section>
      ...
      <div className="flex justify-end gap-2 border-t pt-5"><Button variant="outline" onClick={() => router.push("/payroll/schedules")}>Cancel</Button><Button variant="primary" disabled={save.isPending || !name.trim() || !nextRunAt || !sourceAccountId} onClick={() => save.mutate({
        ...
        weekendPolicy, sourceCurrency, sourceAccountId, fundingReminderDays: Number(fundingReminderDays), personIds,
    ...
    <Card className="h-fit shadow-soft lg:sticky lg:top-6"><CardContent className="p-5"><h2 className="font-semibold">Upcoming paydays</h2><ol className="mt-4 space-y-3">{preview.map((date, index) => <li key={date} className="flex items-center justify-between rounded-xl bg-muted/50 p-3 text-sm"><span>{index === 0 ? "Next" : `After ${index}`}</span><span className="font-medium">{formatDate(date)}</span></li>)}</ol><p className="mt-4 text-xs text-muted-foreground">Dates are adjusted using the selected weekend policy. Holiday calendars can adjust dates during draft creation.</p></CardContent></Card>
  </div>
}
