      ...
      <Card className="shadow-card"><CardContent className="space-y-6 p-6 sm:p-8">
        {person.rail === "easetag" ? (
          <div className="flex items-center gap-3 rounded-xl border p-4">
            ...
            <div><p className="font-medium">{person.fullName}</p><p className="text-xs text-muted-foreground">@{person.easetag?.replace(/^@/, "")} · Identity managed through EASETAG</p></div>
          </div>
        ) : (
