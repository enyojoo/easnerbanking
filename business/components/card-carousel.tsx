    ...
    >
      <div className="absolute inset-0 z-[1] opacity-10">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id={`dots-${card.id}`} x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
              <circle cx="2" cy="2" r="1.5" fill="white" />
            </pattern>
            <pattern id={`lines-${card.id}`} x="0" y="0" width="60" height="60" patternUnits="userSpaceOnUse">
              <path d="M0 60 L60 0" stroke="white" strokeWidth="0.5" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill={`url(#dots-${card.id})`} />
          <rect width="100%" height="100%" fill={`url(#lines-${card.id})`} />
        </svg>
      </div>
...
export function CardCarousel({ cards, onCardChange, frozenCardIds = new Set() }: CardCarouselProps) {
  ...
  if (cards.length === 0) {
    return (
      <div className="relative w-full max-w-[400px] mx-auto">
        <CardFace card={PREVIEW_PLACEHOLDER_CARD} frozen={false} comingSoonOverlay />
      </div>
    )
  }
  ...
  return (
    <div className="relative w-full max-w-[400px] mx-auto">
      <CardFace card={currentCard} frozen={isFrozen} />
