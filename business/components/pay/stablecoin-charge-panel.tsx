...
export type StablecoinChargeSession = {
  id: string
  status: string
...
type Props = {
  sessionId: string
  /**
   * `counter` is the operator taking a payment at their own counter;
...
export function StablecoinChargePanel({
  ...
  useEffect(() => {
    ...
    const endpoint =
      variant === "customer"
        ? `/api/terminal/sessions/public/${encodeURIComponent(sessionId)}`
    ...
    <div className="flex w-full flex-col items-stretch gap-5 text-center sm:gap-6">
      <div className="px-0 sm:px-2">
        ...
        {isPaid ? (
          ...
          <p className="mt-2 text-sm text-destructive">
            This charge could not be completed. Start a new payment.
          </p>
        ) : (
          <p className="mt-2 text-sm text-muted-foreground">
            Send at least the crypto amount below on{" "}
            <span className="font-medium">{session.network}</span>. Sending more may still settle
            depending on your wallet and network fees.
          </p>
        )}
      </div>
        ...
        <>
          ...
          {countdown && !isFailed && session.status !== "payout_complete" ? (
            <p className="text-xs text-muted-foreground">
              {countdown === "Expired"
                ? "This payment window may have expired. If payment already went through, status will update once the deposit is confirmed."
      ...
      <div className="w-full space-y-2 rounded-xl border bg-muted/30 px-4 py-3 text-left text-sm sm:px-5">
        <div className="flex justify-between gap-3">
          <span className="shrink-0 text-muted-foreground">{amountLabel}</span>
