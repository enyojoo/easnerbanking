...
const mobileMoneyProvidersByCurrency: Record<string, string[]> = {
  ...
  GHS: ["AirtelTigo", "MTN Ghana", "Vodafone"],
  MWK: ["Airtel Money", "TNM"],
  RWF: ["Airtel Money", "MTN Rwanda"],
...
export function RecipientForm({
  ...
  const selectedCorridorRow = useMemo(() => {
    ...
    return (
      list.find(
        (c) =>
          c.country_code === selectedCountry.code && c.currency_code === selectedCountry.currency,
      ) ?? null
    )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: RecipientUpsertInput = {
      ...
      email:
        recipientFormNeedsEmail(payoutFormHints) && formData.email.trim()
          ? formData.email.trim()
          : undefined,
      addressLine1:
        isUsdBank ||
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {formData.transferType !== "Wire" ? (
              <div className="space-y-2 col-span-2">
                ...
                {errors.checkingOrSavings && (
                  <p className="text-xs text-red-500">{errors.checkingOrSavings}</p>
                )}
              </div>
            ) : null}
            <div className="space-y-2 col-span-2">
