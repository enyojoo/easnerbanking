...
export type ThroughLocalCurrencyQuoteResult = {
  ok: true
  transferId: string
  ...
  expiresAt: string
  payInNotice?: string
  payInRail?: LocalPayInRail
  sourcePhone?: string
  sourceNetworkName?: string
}

type CrossBorderRateRow = {
...
export function useThroughLocalCurrencyFlow(input: {
  recipientId: string | null
  enabled: boolean
  receiveCurrency: string
  amountEntryMode: "send" | "receive"
  enteredAmount: number
  /** User residence pay-in currency (skips waiting on eligibility). */
  ...
  const [eligibility, setEligibility] = useState<ThroughLocalCurrencyEligibility | null>(null)
  const [eligibilityLoading, setEligibilityLoading] = useState(false)
  const [rates, setRates] = useState<CrossBorderRateRow[]>([])
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const data = (await res.json().catch(() => ({}))) as ThroughLocalCurrencyEligibility & {
          error?: string
        }
        if (cancelled) return
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const res = await fetchWithSession(ratesPath)
        const data = (await res.json().catch(() => ({}))) as { rates?: CrossBorderRateRow[] }
        if (cancelled) return
        if (!res.ok) {
          setRates([])
          return
  ...
  const createQuote = useCallback(
    async (opts: {
      ...
      networkId?: string
      sourceNetworkName?: string
    }): Promise<ThroughLocalCurrencyQuoteResult> => {
      ...
      try {
        ...
        const res = await fetchWithSession(quotePath, {
          method: "POST",
        ...
        const data = (await res.json().catch(() => ({}))) as ThroughLocalCurrencyQuoteResult & {
          error?: string
        }
        if (!res.ok || !data.ok) {
