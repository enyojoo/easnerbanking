-- New Payroll receiving methods are fully owned by Payroll. The legacy
-- provider_recipient_id column remains temporarily readable so existing
-- submitted runs can settle while they are progressively backfilled.

UPDATE payroll_payment_methods
   SET provider_recipient_id = NULL,
       updated_at = now()
...
COMMENT ON COLUMN payroll_payment_methods.provider_recipient_id IS
  'Deprecated legacy recipient link. New Payroll methods use encrypted_details and leave this null.';
