...
import { isRelayConfigured, relayRateProbeTronAddress, resolveCryptoRatesProbeSolAddress } from "../lib/relay/config"
...
const PROBE_TO_TRON = relayRateProbeTronAddress()
...
async function main() {
  ...
  for (const c of corridors) {
    ...
    try {
      ...
      console.log({ ok: true, fromRaw, toHuman, requestId })
      ...
      if (requestId) {
        ...
        if (req) {
          ...
          console.log({ requestLookup: false })
        }
      }
    } catch (e) {
      console.log({ ok: false, error: e instanceof Error ? e.message : String(e) })
    }
  }
}
