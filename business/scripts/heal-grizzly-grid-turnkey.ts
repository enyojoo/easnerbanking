/**
 * One-shot heal: Grizzly Construction Grid KYB approved but Turnkey sub-org /
 * USDC external account never provisioned; VA funds stranded on INTERNAL_FIAT.
 *
 * Usage:
 *   cd business
 *   node --env-file=.env.local --import ./scripts/stub-server-only.mjs --import tsx scripts/heal-grizzly-grid-turnkey.ts
 */
...
const SWEEP_TRANSFER_IDS = [
  "be2a1500-bc27-4f74-b5fc-1f3e8be65539", // $4500
  "b3db1eb6-b632-4717-aa8a-45bcd92425e7", // $1
] as const
...
async function main() {
  ...
  console.log(JSON.stringify({ step: "ensure_sub_org", subOrg }, null, 2))
  ...
  if (!String(deposits.USD?.ownerAddress ?? deposits.USD?.address ?? "").trim()) {
    throw new Error("solana_vault_still_missing")
  }
  ...
  console.log(JSON.stringify({ step: "refresh_rails", rails }, null, 2))
  ...
  for (const transferId of SWEEP_TRANSFER_IDS) {
    const result = await executeGridVaTurnkeySweep(admin, transferId)
    sweeps.push({ transferId, ...result })
  }
  console.log(JSON.stringify({ step: "sweeps", sweeps }, null, 2))
}
