...
/**
 * GET /api/admin/receipts/documents?path=receipts/transactionId.ext
 * 
 * Returns a signed URL for a transaction receipt that admins can use to view files.
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isAdmin) {
      console.log(`Admin ${adminUser.email} requesting receipt: ${filePath}`)
    } else {
      console.log(`[ADMIN-RECEIPTS] Admin check failed, using service role for: ${filePath}`)
    }
    ...
    console.log(`[ADMIN-RECEIPTS] Generating signed URL with service role for: ${filePath}`)
    const { data, error } = await serverClient.storage
      .from("transaction-receipts")
      .createSignedUrl(filePath, 3600) // 1 hour expiry
    
    if (error) {
      console.error("[ADMIN-RECEIPTS] Error generating signed URL:", {
        error: error.message,
    ...
    console.log(`[ADMIN-RECEIPTS] Successfully generated signed URL for ${filePath}`)
    
    // Return the signed URL
    return NextResponse.json({ url: data.signedUrl })
  } catch (error: any) {
    console.error("Error in admin receipt document endpoint:", {
      error: error.message,
      stack: error.stack
    })
